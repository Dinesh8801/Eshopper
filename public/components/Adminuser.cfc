component {
	...
	remote struct function addCoupon(string couponCode, numeric discount) returnformat="JSON" {
		...
		if (len(trim(arguments.couponCode)) EQ 0 || len(trim(arguments.discount)) EQ 0) {
			result.success = false;
			result.message = "All fields are mandatory.";
			return result;
		}
	
		var existingCouponCheck = queryExecute(
			"SELECT CouponCode FROM Coupons WHERE CouponCode = :couponCode",
			{
		...
		if (existingCouponCheck.recordCount > 0) {
			result.success = false;
			result.message = "Coupon code already exists.";
			return result;
		}
	
		try {
			queryExecute(
				"INSERT INTO Coupons (CouponCode, Discount) VALUES (:couponCode, :discount);",
				{
					couponCode = couponCode,
					discount = discount
				},
	...
	remote struct function updateCoupon(numeric couponId, string couponCode, numeric discount) returnformat="JSON" {
		...
		if (len(trim(arguments.couponCode)) EQ 0) {
			result.SUCCESS = false;
			result.MESSAGE = "Coupon Code is mandatory.";
			return result;
		}
		if (arguments.discount LTE 0) {
			result.SUCCESS = false;
			result.MESSAGE = "Discount must be a positive number.";
			return result;
		}
	
		var existingCouponCheck = queryExecute(
			"SELECT CouponID FROM Coupons WHERE CouponCode = :couponCode AND CouponID != :couponId",
			{ 
		...
		try {
			queryExecute(
				"UPDATE Coupons SET CouponCode = :couponCode, Discount = :discount WHERE CouponID = :couponId",
				{
					couponCode = arguments.couponCode,
	...
	remote struct function getCouponById(numeric couponId) returnformat="JSON" {
		var result = {};
		try {
			var couponQuery = queryExecute(
				"SELECT couponId, couponCode, discount FROM Coupons WHERE couponId = ?",
				[couponId],
				{ datasource = "martDSN" }
			);
	
			if (couponQuery.recordCount eq 1) {
				var row = couponQuery.getRow(1);
				result = {
	...
	remote struct function deleteCoupon(numeric couponId) returnformat="JSON" {
		...
		try {
			// Perform the DELETE operation using queryExecute
			queryExecute(
			...
			result = { success = "false", error = "Error: " & e.message & " | StackTrace: " & e.stackTrace };
		}
	...
	remote struct function getCouponsForAdmin() returnformat="JSON" {
		result = {}; // Initialize an empty struct

	// Query to fetch coupon data
	getCoupons = queryExecute(
		"SELECT couponId, couponCode, discount FROM Coupons", 
		[], 
		{datasource="martDSN"}
	);

	// Check if any records exist
	if (getCoupons.recordCount EQ 0) {
		result.ERROR = "No coupons found.";
		return result;
	}

	// Loop through the query and populate the result struct
	for (i = 1; i <= getCoupons.recordCount; i++) {
		result[getCoupons.couponId[i]] = {
			"couponCode" = getCoupons.couponCode[i],
			"discount" = getCoupons.discount[i]
		};
	}
